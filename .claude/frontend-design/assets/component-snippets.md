...
Ready-to-use component code for common UI patterns.

## Buttons

### Button Component (Tailwind + React)
    ...
    return (
      <div className="space-y-1.5">
        ...
        <select
          ...
          id={selectId}
          className={cn(
            'block w-full px-3 py-2 border rounded-lg bg-white text-gray-900',
            'focus:outline-none focus:ring-2 focus:ring-offset-0',
            error
              ? 'border-red-500 focus:ring-red-500'
...
function Alert({ variant = 'info', title, children, onClose }: AlertProps) {
  ...
  return (
    <div className={cn('flex gap-3 p-4 border rounded-lg', variants[variant])} role="alert">
      {icons[variant]}
      ...
      {onClose && (
        <button onClick={onClose} className="self-start opacity-70 hover:opacity-100">
